...
func (adapter *Adapter) Stream(logstream chan *router.Message) {
	...
	for m := range logstream {
		...
		var meta string
		var extraInfo ExtractDataInfo
		_ = json.Unmarshal([]byte(m.Data), &extraInfo) // we ignore if there were an error when unmarshalling data as it might be non json format
		...
		if extraInfo.Meta != nil {
			metaInfoBytes, err2 := json.Marshal(extraInfo.Meta)
			if err2 != nil {
				adapter.Logger.Println(
					fmt.Errorf(
						"JSON Marshalling Error: %s when when decoding meta info: %v",
						err2.Error(),
						extraInfo.Meta,
					),
				)
			} else {
				meta = string(metaInfoBytes)
			}
		}
